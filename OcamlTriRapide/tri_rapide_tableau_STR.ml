...
code natif Windows : 
with-dkml ocamlopt -o str.exe tri_rapide_tableau_STR.ml
...
(* let compare_a_a_renvoie_bool x y = 
  if x < y then true else false;; *)

let compare_a_b_renvoie_bool a b =
   let la = String.length a and
   lb =  String.length b in
   if la < lb then true else
   if la == lb then
